...
function TimelineCell({
  ...
  if (!lines.length) {
    return (
      <div className="flex min-h-16 items-center justify-center px-2 py-3 text-[10px] text-muted-foreground">
        {snapshot.status === "pending" ? "待收集" : "沒有記錄"}
      </div>
    );
  }
  return (
      ...
      {lines.map((line, index) => (
        <div key={line.lineKey} className={cn(index > 0 && "border-t border-grid pt-1.5")}>
          ...
          <div className="mt-1 text-[10px] leading-4 text-muted-foreground">
            <span className="block">來源 {fmtTime(line.prices[0].sourceUpdatedAt)}</span>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 bg-muted/20 px-3 py-2 text-[10px] text-muted-foreground">
        <span>初盤為系統首次成功觀察盤；每個階段收集後鎖定，不會被最新賠率覆蓋。</span>
    ...
    <div className="flex min-h-[100dvh] flex-col bg-background text-foreground">
      <header className="sticky top-0 z-20 shrink-0 border-b border-border bg-card/95 backdrop-blur">
        <div className="flex flex-wrap items-center gap-2 px-3 py-2">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-[10px] text-muted-foreground">馬會 × Pinnacle · 初盤 / T-30 / T-15 / T-5 / 賽果</p>
          </div>
        ...
        <div className="grid grid-cols-2 divide-x divide-y divide-border border-t border-border sm:grid-cols-4 sm:divide-y-0">
          ...
          <div className="px-3 py-2"><p className="text-[10px] text-muted-foreground">初盤覆蓋</p><p className="tnum text-lg font-semibold" data-testid="kpi-research-initial">{data ? percent(coverage("initial")) : "—"}</p></div>
      ...
      <main className="flex-1 space-y-2 overflow-y-auto p-2 sm:p-3" data-testid="scroll-research">
        {isLoading ? <TableSkeleton rows={8} /> : isError ? (
          ...
          <EmptyState title="呢個篩選暫時未有數據" hint="系統會在背景自動收集四個時間點及完場賽果。" testId="empty-research" />
