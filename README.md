...
* **研究時間線**：以獨立資料表鎖定馬會 × Pinnacle 的莊家真初盤、T-30、T-15、T-5，涵蓋亞洲讓球、入球大細及角球大細。真初盤只從可核實的外部開盤紀錄匯入並保留來源證據，不會用 Radar 首次觀察盤代替；目前無可核實 Pinnacle 角球真初盤時會明確留空。T-30、T-15、T-5 由 Radar 自動收集，每筆保留來源時間與收集時間，完整及部分樣本分開標示。隔離收集器另行補上 HKJC 官方比分及角球結果，不會改動模擬結算、Wilson、Telegram 或落注流程。
...
| `RADAR_RESEARCH_OPENING_INTERVAL_MINUTES` | `30` | 真初盤來源檢查間隔，最少 5 分鐘 |
| `RADAR_RESEARCH_OPENING_LOOKBACK_DAYS` | `2` | 真初盤配對向後檢查日數，最多 30 日 |
| `RADAR_RESEARCH_OPENING_AHEAD_DAYS` | `7` | 真初盤配對向前檢查日數，最多 30 日 |
