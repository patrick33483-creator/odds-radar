 ...
 *
 * Recovered formulas (away/受讓 side, using HKJC draw price O_D, away price O_A):
 *
 ...
 *
 *  3. 合成「受讓 +0.25」(受讓半平)
 *       ½W by the +0 formula, ½W by the +0.5 formula
 *       O_syn(+0.25) = ( O_syn(+0) + O_syn(+0.5) ) / 2
 *
 *  4. 合成「受讓 +0.75」(受讓半一)
 *       ½W by the +0.5 formula, ½W on the OFFICIAL 讓球 +1.0 price O_(+1.0)
 *       O_syn(+0.75) = ( O_syn(+0.5) + O_(+1.0) ) / 2
 *
 * The home/讓球 side mirrors these with O_H substituted for O_A.
 */
...
export type SynTarget = (typeof SYNTHETIC_TARGETS)[number];
...
export function buildSynthetic(
  ...
  const { oddsDraw: oD, official1 } = inputs;
  ...
  if (target === 0.5) {
    ...
    formula = "O_syn(+0.25) = ( O_syn(+0) + O_syn(+0.5) ) / 2";
  } else {
  ...
  return { side, target, homeHandicap, lineDisplay, odds: r3(odds), formula, components };
}
