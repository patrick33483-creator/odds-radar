/**
 * Expected value (正期望值) using Crown no-vig probabilities as the true-probability
...
/** A price this far from the Crown fair price is treated as suspicious. */
...
export interface EvInput {
  ...
  lineKey: string;
  lineDisplay: string;
  /** Crown prices for the full complementary set of the same exact line. */
  crown: Array<{ selection: Selection; decimalOdds: number }>;
...
/**
 * Evaluate every HKJC selection on one exact line against the Crown no-vig fair
...
export function evaluateEv(input: EvInput): EvOpportunity[] {
  ...
  if (input.crown.length < 2) return [];
  const probs = noVigProbs(input.crown.map((c) => c.decimalOdds));
  if (!probs) return [];
  const fairBySelection = new Map<Selection, number>();
  input.crown.forEach((c, i) => fairBySelection.set(c.selection, probs[i]));
