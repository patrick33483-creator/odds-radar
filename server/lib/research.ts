...
/** The three stages a rescue pass can rebuild from titan007 history. */
export type BackfillStage = "T30" | "T15" | "T5";
export const BACKFILL_STAGES: BackfillStage[] = ["T30", "T15", "T5"];

/**
 * The exact observation window that makes `researchStageFor` return `stage`,
 * as a half-open `[from, to)` interval of wall-clock instants. Derived from
 * the same minute bounds so the two can never drift apart:
 *   T30 -> minutes-to-kickoff in (15, 30]  -> [kickoff-30m, kickoff-15m)
 *   T15 -> minutes-to-kickoff in ( 5, 15]  -> [kickoff-15m, kickoff-5m)
 *   T5  -> minutes-to-kickoff in ( 0,  5]  -> [kickoff-5m,  kickoff)
 */
export function researchStageWindow(
  stage: BackfillStage,
  kickoffUtc: number,
): { from: number; to: number } {
  const bounds: Record<BackfillStage, [number, number]> = { T30: [30, 15], T15: [15, 5], T5: [5, 0] };
  const [fromMinutes, toMinutes] = bounds[stage];
  return { from: kickoffUtc - fromMinutes * 60_000, to: kickoffUtc - toMinutes * 60_000 };
}

export interface StageHistoryPick<Row> {
  row: Row;
  /** True when the tick itself falls inside the stage window. */
  inWindow: boolean;
}

/**
 * Choose the tick that represents `stage` from a titan007 history table.
 *
 * titan only records a row when the price MOVED, so a quiet T-15 window has no
 * row of its own even though a quote was live the whole time. Preference order:
 *   1. the LATEST pre-match tick inside the stage window (exact evidence);
 *   2. otherwise the latest pre-match tick strictly before the window end
 *      (last-observation-carried-forward — that quote was still the standing
 *      price during the window).
 * In-play ticks are never eligible, so a live price can never be relabelled as
 * a pre-match checkpoint.
 */
export function selectStageHistoryRow<Row extends { timestamp: number; prematch: boolean }>(
  rows: readonly Row[],
  kickoffUtc: number,
  stage: BackfillStage,
): StageHistoryPick<Row> | null {
  const { from, to } = researchStageWindow(stage, kickoffUtc);
  const prematch = rows.filter((row) => row.prematch).sort((a, b) => a.timestamp - b.timestamp);
  const inside = prematch.filter((row) => row.timestamp >= from && row.timestamp < to);
  if (inside.length) return { row: inside[inside.length - 1], inWindow: true };
  const carried = prematch.filter((row) => row.timestamp < to);
  if (carried.length) return { row: carried[carried.length - 1], inWindow: false };
  return null;
}
...
/**
 * Write a rescued stage checkpoint recovered from titan007 history.
 *
 * Strictly additive: `INSERT OR IGNORE` on the (match,provider,market,stage,
 * line,selection) unique index means an already-captured stage can never be
 * overwritten or re-priced, and `captured_at` carries the HISTORICAL tick
 * timestamp (not "now") so the timeline stays honest. `origin='backfill'`
 * keeps rescued rows auditable against live observations.
 */
export function saveResearchStageBackfill(
  matchId: string,
  provider: "pinnacle" | "crown",
  stage: BackfillStage,
  prices: ProviderPrice[],
  kickoffUtc: number,
  capturedAt: number,
  source: { name: string; matchId: string | null; url: string },
): number {
  ...
  let inserted = 0;
  rawDb.transaction(() => {
    ensurePoint.run(matchId, stage, stageTargetAt(stage, kickoffUtc), capturedAt, capturedAt);
    for (const price of accepted) {
      inserted += insert.run(
        matchId,
        provider,
    ...
    const expected = expectedPairCount(matchId, stage);
    const status = complete >= expected ? "captured" : "partial";
    const note = status === "captured"
      ? null
      : `${complete}/${expected} provider-market pairs complete`;
    rawDb.prepare(
      `UPDATE research_timeline_points
          SET first_captured_at=COALESCE(first_captured_at,?),
              captured_at=COALESCE(captured_at,?),
              status=CASE WHEN status='captured' THEN 'captured' ELSE ? END,
              note=CASE WHEN status='captured' THEN note ELSE ? END,
              updated_at=?
        WHERE match_id=? AND stage=?`,
    ).run(capturedAt, capturedAt, status, note, Date.now(), matchId, stage);
  })();
  return inserted;
}
